...
const props = defineProps<{
  values: Array<{ category: string; value: number }>
}>()
...
<template>
  <Card class="border-border/70 bg-card/80">
    <CardHeader>
      <CardTitle class="text-sm font-medium text-muted-foreground">Distribuicao por categoria</CardTitle>
    </CardHeader>
    <CardContent class="space-y-6">
      <div class="mx-auto size-44 rounded-full border border-border/70" :style="{ background: gradient }" />

      <div class="space-y-2">
        <div v-for="(item, idx) in values" :key="item.category" class="flex items-center justify-between text-sm">
